...
app.post('/register', async(req, res) => {
	try{
		...
		const {email, username, password} = req.body.data; // Extracting 'email', 'username', and 'password' from the request body
		...
		if(data.email === null || email === null) // Checking if the data is null or undefined
			res.send(false) // Sending 'false' response if the data is invalid
		if(req.body.data.email === data.email) // Checking if the provided email matches the data
			res.send(false) // Sending 'false' response if the email already exists
		else {
			...
			res.send(true) // Sending 'true' response if the registration is successful
		}
	} catch (error) {
